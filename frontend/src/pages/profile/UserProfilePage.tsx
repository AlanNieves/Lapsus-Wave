...
const UserProfilePage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const meRes = await axiosInstance.get(`${BASE_URL}/users/me`, {
        ...
        const postRes = await axiosInstance.get(`${BASE_URL}/posts`, { withCredentials: true });
        setPosts(postRes.data.filter((p: UserPost) => p.userId?._id === id));
      } catch (err) {
  ...
  const handleFollowToggle = async () => {
    try {
      const res = await axiosInstance.post(`${BASE_URL}/follow/${id}`, {}, {
        withCredentials: true,
      });
    ...
    try {
      const results = await Promise.all(
        user.followers.map((followerId) =>
          axiosInstance.get(`${BASE_URL}/users/${followerId}`, { withCredentials: true })
        )
      );
                  ...
                  {posts.map((post) => (
                    <div key={post._id} className="bg-black/20 p-4 rounded-lg border border-white/10">
                      <div className="flex items-center gap-3 mb-2">
                        <img src={post.userId?.avatar || "/default-avatar.png"} alt="User" className="w-8 h-8 rounded-full object-cover" />
